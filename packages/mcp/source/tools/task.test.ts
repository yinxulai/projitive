...
describe('tasks module', () => {
  ...
  it('renders tasks with subState, blocker metadata, acceptance criteria, and dependencies', () => {
    ...
    const task3 = normalizeTask({
      ...
      title: 'Delivery Task',
      status: 'TODO',
      acceptanceCriteria: ['Implement parser', 'Add regression tests'],
      dependencies: ['TASK-0002'],
    })

    const markdown = renderTasksMarkdown([task1, task2, task3])
  ...
  it('taskCreate auto-generates the next task id and syncs tasks view', async () => {
    ...
    const result = await taskCreate({
      ...
      roadmapRefs: ['ROADMAP-0001'],
      acceptanceCriteria: ['Ship the change', 'Verify the regression path'],
    ...
    expect(loaded.tasks.find((task) => task.id === 'TASK-0010')?.acceptanceCriteria).toEqual(['Ship the change', 'Verify the regression path'])
    expect(loaded.tasks.find((task) => task.id === 'TASK-0010')?.dependencies).toEqual(['TASK-0009'])
